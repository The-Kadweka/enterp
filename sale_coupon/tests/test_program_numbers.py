...
class TestSaleCouponProgramNumbers(TestSaleCouponCommon):
    ...
    def test_coupon_and_program_discount_fixed_amount(self):
        # Ensure coupon and program discount can cohexists
        order = self.empty_order
        testprod = self.env['product.product'].create({
            'name': 'testprod',
            'lst_price': 16.5,
    ...
    def test_coupon_and_coupon_discount_fixed_amount_tax_incl(self):
        ...
        self.assertEqual(order.amount_total, 9.09, "The promotion program should not go below 0")
    ...
    def test_program_free_prods_with_min_qty_and_reward_qty_and_rule(self):
        order = self.empty_order
        coupon_program = self.env['sale.coupon.program'].create({
            'name': '2 free iPod if at least 1 iPad Mini',
            'promo_code_usage': 'code_needed',
            'program_type': 'promotion_program',
            'reward_type': 'product',
            ...
            'reward_product_id': self.iPod.id,
            'rule_min_quantity': 1,
            'rule_products_domain': '["&", ["sale_ok","=",True], ["name","ilike","ipad mini"]]',
        })
        # set ipad and ipod prices
        self.iPadMini.write({'list_price': 500, 'name': 'iPad Mini'})
        self.iPod.write({'list_price': 100})
        ...
        # Name                 | Qty | price_unit |  Tax     |  HTVA   |   TVAC  |  TVA  |
        # --------------------------------------------------------------------------------
        # Ipod                 |  2  |    100.00  | /        |  200.00 |  200.00 |       /
        # iPad                 |  1  |    500.00  | /        |  500.00 |  500.00 |       /
        #
        # Free iPod            |  2  |   -100.00  | /        | -200.00 | -200.00 |       /
        # --------------------------------------------------------------------------------
        # TOTAL                                              |  500.00 |  500.00 |       /

        self.assertEqual(len(order.order_line), 3, 'The order must contain 3 order lines including one for Free iPod')
        self.assertEqual(order.amount_total, 500.0, 'The price must be 500.0 since two iPod are free')
        self.assertEqual(order.order_line[2].price_total, -200.0, 'The last order line should apply a reduction of 200.0 since there are two ipods that cost 100.0 each')
        ...
        # iPad                 |  1  |    500.00  | /        |  500.00 |  500.00 |       /
        #
        # Free iPod            |  1  |   -100.00  | /        | -100.00 | -100.00 |       /
        # --------------------------------------------------------------------------------
        # TOTAL                                              |  500.00 |  500.00 |       /

        self.assertEqual(order.amount_total, 500.0, 'The price must be 500.0 since two iPod are free and the user only bought one')
        self.assertEqual(order.order_line[2].price_total, -100.0, 'The last order line should apply a reduction of 100.0 since there is one ipod that cost 100.0')
