...
class AccountBatchPayment(models.Model):
    ...
    def validate_batch(self):
        ...
        for record in records:
            if any(payment.state != 'posted' for payment in record.payment_ids):
                raise ValidationError(_("You cannot validate a batch containing payments that are not in 'posted' state."))
