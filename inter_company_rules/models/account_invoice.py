...
class account_invoice(models.Model):
    ...
    def inter_company_create_invoice(self, company, inv_type, journal_type):
        ...
        for line in self.invoice_line_ids.filtered(lambda l: l.display_type == False):
            ...
            inv_line_data['account_analytic_id'] = self.sudo()._get_analytic_account(line, company)
            ...
            # using the function _onchange_product_id can lead to some issues if the same taxes are not applied
    ...
    def _get_analytic_account(self, line, company):
        if line.account_analytic_id:
            child_companies = self.env['res.company'].search([('id', 'child_of', company.id)])
            return line.account_analytic_id.id if line.account_analytic_id.company_id in child_companies else False
        return False
