...
class TestMrpPlm(common.TransactionCase):
    ...
    def test_do_not_merge_bom_lines(self):
        ...
        routing = self.env['mrp.routing'].create({'name': 'Simple Line'})
        ...
        self.assertEqual(len(mrp_eco.bom_change_ids), 1, "Only one modification was made")
        self.assertRecordValues(mrp_eco.bom_change_ids[0], [{  # We are updating one line by adding 11 to it
            'change_type': 'update',
            'upd_product_qty': 11,
        }])
