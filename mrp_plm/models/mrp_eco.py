...
class MrpEco(models.Model):
    ...
    def _get_difference_bom_lines(self, old_bom, new_bom):
        ...
        new_bom_commands = []
        old_bom_lines = dict(((line.product_id, tuple(line.attribute_value_ids.ids), line.operation_id), line) for line in old_bom.bom_line_ids)
        if self.new_bom_id:
            for line in new_bom.bom_line_ids:
                old_line = old_bom_lines.pop((line.product_id, tuple(line.attribute_value_ids.ids), line.operation_id), None)
