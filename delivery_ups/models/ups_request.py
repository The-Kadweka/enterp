    ...
    def check_required_value(self, shipper, ship_from, ship_to, order=False, picking=False):
        ...
        if picking:
            ...
            for ml in picking.move_line_ids.filtered(lambda ml: not ml.result_package_id and not ml.product_id.weight):
                line = picking.move_line_ids.filtered(lambda ml: not ml.result_package_id and not ml.product_id.weight)[0]
                return _("The delivery cannot be done because the weight of your product %s is missing.") % line.product_id.display_name
